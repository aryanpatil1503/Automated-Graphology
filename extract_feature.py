...
def extractLines(img):
    ...
    print(
        "Average line spacing relative to average letter size: "
        + str(relative_line_spacing)
    )
...
def extractWords(image, lines):
        ...
        for j, sum in enumerate(vp):
            ...
            if sum > 0:
                ...
                for k in range(int(line[1]) - int(line[0])):
                    row = thresh[
                        line[0] + k: line[0] + k + 1, wordStart:wordEnd
                    ]  # y1:y2, x1:x2
                    if np.sum(row):
                        count += 1
    ...
    print(
        "Average word spacing relative to average letter size: "
        + str(relative_word_spacing)
    )
...
def extractSlant(img, words):
    ...
    for i, angle in enumerate(theta):
        s_temp = (
            0.0  # overall sum of the functions of all the columns of all the words!
        )
        ...
        for j, word in enumerate(words):
            original = thresh[
                int(word[0]): int(word[1]), int(word[2]): int(word[3])
            ]  # y1:y2, x1:x2
            ...
            blank_image = np.zeros(
                (height, width + pad_length * 2, 3), np.uint8)
            ...
            pts2 = np.float32(
                [[x1 + shift, y1], [x2 - shift, y2], [x3 - shift, y3]])
...
def main():
    ...
    image = cv2.imread(
        "C:\\Users\\aryan\\Desktop\\TE SEM5\\Seminar\\project\\virtual_env\\images\\aryan.jpeg")
    ...
    #   cv2.imshow("item " + str(i),
    #             image[item[0]: item[1], item[2]: item[3]])
